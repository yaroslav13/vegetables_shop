...
class NutsBloc extends BaseBloc{
  ...
  List<Product> allNuts = List<Product>();
  ...
  void _getSpices(List<Product> allProducts) {
    const nutsType = ProductType.nuts;
    List<Product> nuts = List<Product>();

    for(var product in allProducts){
      if(product.productType == nutsType){
        nuts.add(product);
      }
    }

    allNuts.addAll(nuts);
    searchProducts.add(nuts);
  }
}